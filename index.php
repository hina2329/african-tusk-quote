...
//        \_WP_Editors::editor_js();
    ...
    public function add_category() {
       ...
       $cats = $this->wpdb->get_results("SELECT cat.*, prod.* FROM $this->categories_relation_tbl AS cat INNER JOIN $this->products_tbl AS prod ON cat.prod_id = prod.prod_id WHERE cat.cat_id = $item_cat");
       foreach ($cats as $cat) {
          // Save existing product data into wp_atq_quote_items table
        $product_data = array(
            'item_qid' => $quote_id,
            'item_pid' => $cat->prod_id,
            'item_code' => $cat->prod_code,
            'item_images' => $cat->prod_images,
            'item_name' => $cat->prod_name,
            'item_desc' => $cat->prod_desc
         
        );

        $this->wpdb->insert($this->quote_items_tbl, $product_data);
        $item_id = $this->wpdb->insert_id;
        $images = unserialize($cat->prod_images);
        $textarea_id = 'desc' . $cat->prod_id;
        echo '<tr>';
        echo '<td>';
        ...
        $item = $this->wpdb->get_row("SELECT * FROM $this->quote_items_tbl WHERE item_id = $item_id");
        echo '<input type="text" name="item[' . $item_id . '][name]" value=" ' . $cat->prod_name . '">';
        echo '</td>';
        echo '<td>';

      

        echo '<textarea name="item[' . $item_id . '][desc]" rows="10">' . $cat->prod_desc . '</textarea>';
        ...
        //getting fabric suffix
        $prod_fps = $this->wpdb->get_results("SELECT * FROM $this->products_fp_combos_tbl WHERE combo_pid = $cat->prod_id");
        foreach ($prod_fps as $prod_fp) {
